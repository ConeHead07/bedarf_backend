...
<script>
    ...
    var aRows = <?= json_encode($tplVars->aRows, JSON_PRETTY_PRINT) ?> || [];
</script>
...
<script>
    ...
    var listConf = {
        ...
        rownumbers: true,
        colfilters: true,
        title: 'Inventuren mit Objektbuchauswahl',
        fields: {
            jobid: { name: 'Job-ID' },
            mid: { name: 'MID' },
            Titel: { name: 'Titel' },
            Start: { name: 'Startdatum' },
            aktiviert: { name: 'Aktiv', formatter: function(val, colname, rowElm, rowData) {
                    $(this).text( parseInt(val) === 1 ? 'Ja' : 'Nein' );
                } },
            created: { name: 'Upload vom' },
            x: { name: 'hallo', formatter: function(val, colname, rowElm, rowData) {
                $(this).html(
                    '<a href="/api/admin/objektbuch/' + rowData.jobid + '">Objektbuch laden</a>'
                )
                } }
        }
    };
...
<script>
    ...
    if (0) function dataTable( selector, data, props) {
            ...
            var renderer = {
                ...
                renderFoot: function() {
                    if (('showFooter' in config) && !config.showFooter) {
                        alert("listBooks.php cancelled renderFoot");
                        return;
                    }
    ...
    $('#tableList').myDataTable( listConf).orderby('created_at', 'DESC');
